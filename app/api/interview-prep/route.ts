...
import { generateTextForUser } from "@/utils/gemini";
import { buildInterviewPrepPrompt } from "@/utils/interviewPrep";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!hasOwnKeys) {
      try {
        const { addCredits: refundCredits } = await import("@/lib/credits");
