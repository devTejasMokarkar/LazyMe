      ...
      if (res.ok) {
        ...
        const saveRes = await fetch('/api/resumes', {
          ...
          body: JSON.stringify({
            ...
            content: { name: data.name, title: data.title, experience: data.experience, skills: data.skills, email: data.email, phone: data.phone },
            isDefault: true
          })
        });
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-6">
          <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="bg-surface-container-low border border-outline-variant/50 rounded-xl p-5 group">
              <h1 className="text-2xl font-bold text-on-surface outline-none focus:text-primary transition-colors" contentEditable suppressContentEditableWarning onBlur={(e) => setUserName(e.currentTarget.innerText)}>{userName}</h1>
              <p className="text-on-surface-variant text-sm font-medium mt-1 outline-none" contentEditable suppressContentEditableWarning onBlur={(e) => setUserRole(e.currentTarget.innerText)}>{userRole}</p>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <button className="p-1 text-on-surface-variant hover:text-primary transition-colors"><PlusCircle className="w-5 h-5" /></button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 flex justify-center bg-[#0d0e13]">
              <motion.div 
                className={cn(
                  ...
                  previewMode === 'mobile' ? "w-[320px]" : "w-[420px]"
                )}
                style={{ 
              ...
              >
                ...
                <div className="text-center mb-6">
                  <h2 className={cn("text-2xl font-extrabold", resumeTheme === 'light' ? "text-slate-900" : "text-white")}>{userName}</h2>
                  <p className={cn("text-sm", resumeTheme === 'light' ? "text-slate-500" : "text-slate-400")}>{userRole}</p>
                </div>
                <div className={cn("flex justify-between text-[9px] uppercase tracking-wider mb-5 pb-3 border-b", resumeTheme === 'light' ? "text-slate-400 border-slate-100" : "text-slate-500 border-slate-800")}>
                  <span>{email || 'email'}</span><span>{phone || 'phone'}</span><span>India</span>
                </div>
